...
应用只生成文本，不会修改目标项目、续跑源任务或自动发送结果。它使用你当前 Codex CLI 的登录状态，不要求单独配置模型 API Key。ChatGPT 私有链接目前只做解析和诚实的不可访问提示，不能承诺读取任意云端历史。`chatgpt.com/s/...` 是共享任务链接，不是对话分享链接；对话分享链接应使用 `chatgpt.com/share/...`。
...
- ChatGPT `/c/...` 和 `/share/...` 暂不自动读取历史；`/s/...` 会明确识别为共享任务链接，不会误称为对话历史。
